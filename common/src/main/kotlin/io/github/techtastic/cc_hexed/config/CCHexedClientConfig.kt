package io.github.techtastic.cc_hexed.config
...
object CCHexedClientConfig {
    ...
    fun init() {
        ...
        holder.registerSaveListener { _, config ->
            CCHexedServerConfig.holder.config = CCHexedServerConfig.GlobalConfig(config.server)
            InteractionResult.PASS
        }

        // when we leave a server, clear our local copy of its config
        ClientPlayerEvent.CLIENT_PLAYER_QUIT.register { _ ->
            CCHexedServerConfig.onSyncConfig(null)
        }
    }

    @Config(name = CCHexed.MODID)
    class GlobalConfig : GlobalData() {
        @Category("server")
        @TransitiveObject
        val client = ClientConfig()

        // this should only be used inside of this class; use CCHexedServerConfig.config to access the server-side config in other code
        @Category("server")
        @TransitiveObject
        val server = CCHexedServerConfig.ServerConfig()
    }
